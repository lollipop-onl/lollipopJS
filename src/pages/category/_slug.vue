  ...
  div(v-if="category")
    ...
    ol
      li(v-for="post in blogPosts")
        ...
        p {{ $store.state.count }}
    AppPagination(
      :to="$utils.url($C.PAGES.BLOG_CATEGORY, { slug })"
...
export default class CategoryPage extends Vue {
  ...
  /** カテゴリ情報 */
  get category(): Entry<BlogCategory> | undefined {
    return this.$contentful.getCategory(this.slug);
  }

  /** ページに表示するブログポスト */
  get blogPosts(): Array<Entry<BlogPost>> {
    return this.$contentful.getCategoryBlogPosts(this.slug, this.page - 1);
  }
